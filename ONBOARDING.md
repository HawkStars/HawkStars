...
**Collections** (declaration order in `payload.config.ts`): `Users` (admin auth, a single `isAdmin` role flag rather than a separate roles collection), `Media`, `Documents`, `ArtCollection` (artworks), `BoardMember`, `Contribution`, `Curator`, `HawkProject`, `HawkEvent`, `MemberProject`, `Partner`, `Sponsor`, `Pages`, `News`, `Notification`.
...
**Access control** (`payload/access/`): three composable functions — `anyone`, `authenticated`, `authenticatedAdmin` (`user.isAdmin`) — plus `authenticatedOrPublished` for the drafts-enabled collections. Every account belongs to a person associated with the NGO and is created by an admin (there is no public sign-up), and there are two tiers: **User** (any account — creates, edits and publishes all content) and **Admin** (`isAdmin` — adds deletes, users, settings and donations). There is no editor tier; the `isEditor` flag and `authenticatedEditor` were removed in September 2026 because with two tiers they were identical to `authenticated`. `.claude/access-control.md` is the canonical description.

**`Users` collection**: just `name` (text) plus one checkbox, `isAdmin` (default `false`, shown in the admin UI only to an admin viewer)` — so they are in fact hidden on the new-user form, which blocks granting a role through the intended UI). `create`/`update`/`delete` require `authenticatedAdmin`; `read`/admin-panel access require any `authenticated` user. Auth config: 30-day token expiration, lockout after 5 failed attempts for 24 hours, cookies forced `secure` in production with `sameSite: 'Lax'`. Two custom hooks: `afterLogin` logs login activity, and `beforeLogin` rate-limits by IP (a separate, coarser guard than the per-account lockout above).
