...
**Access control** (`payload/access/`): four composable functions — `anyone`, `authenticated`, `authenticatedAdmin` (`user.isAdmin`), `authenticatedEditor` (`user.isEditor || user.isAdmin`). Every account belongs to a person associated with the NGO and is created by an admin — there is no public sign-up — so there are three tiers: **Normal** (neither flag; intended to be read-only in `/admin`), **Editor** (manages content), **Admin** (full control, including users, settings and financial records). Note that `authenticated` grants to *all three* tiers, so it is rarely the right choice for a write operation. `.claude/access-control.md` is the canonical description, including the cases where the current code does not yet match this model.

**`Users` collection**, confirmed field-by-field: just `name` (text) plus two checkboxes, `isAdmin` and `isEditor` (both default `false`; both are *intended* to appear in the admin UI only for an admin viewer, but the `condition` reads the record being edited rather than the viewer — Payload's signature is `(data, siblingData, { user })` — so they are in fact hidden on the new-user form, which blocks granting a role through the intended UI). `create`/`update`/`delete` require `authenticatedAdmin`; `read`/admin-panel access require any `authenticated` user. Auth config: 30-day token expiration, lockout after 5 failed attempts for 24 hours, cookies forced `secure` in production with `sameSite: 'Lax'`. Two custom hooks: `afterLogin` logs login activity, and `beforeLogin` rate-limits by IP (a separate, coarser guard than the per-account lockout above).
