...
- <strike>**SEO-M2** — No `app/robots.ts`; only a static `public/robots.txt` that does not disallow `/*/preview/` or `/api/`. The preview routes do set `robots: 'noindex, nofollow'` per page, so this is crawl budget rather than indexation.</strike>
