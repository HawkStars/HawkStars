...
const CrowdfundingFAQ = ({ t }: Props) => {
  return (
    <section id='faq' className='bg-crowdfunding-bg relative w-full overflow-hidden py-16'>
      ...
      />
      <div className='from-crowdfunding-bg absolute top-0 right-0 h-full w-1/3 bg-linear-to-r to-transparent' />
      ...
      <div className='relative mx-auto max-w-7xl px-4 lg:px-8'>
        <div className='mb-12'>
          ...
          <p className='mt-4 max-w-2xl text-sm leading-relaxed text-gray-400'>
            {t('faq.description_prefix')} <span className='text-orange-500'>Global Village</span>.
          </p>
        </div>
        ...
        <div className='grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4'>
          {faqKeys.map((key, index) => (
            <div
              key={key}
              className='group bg-crowdfunding-surface flex flex-col rounded-2xl border border-white/10 p-5 transition hover:border-orange-500/30'
            >
