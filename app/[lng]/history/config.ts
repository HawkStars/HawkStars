import { SlideshowImage } from '@/components/utils/Slideshow/types';

// Images
import humanitarian_1 from '@/public/images/history/humanitarian_1.jpeg';
import humanitarian_2 from '@/public/images/history/humanitarian_2.jpg';
import humanitarian_3 from '@/public/images/history/humanitarian_3.jpg';
import humanitarian_4 from '@/public/images/history/humanitarian_4.jpg';
import humanitarian_5 from '@/public/images/history/humanitarian_5.jpg';

const historyReferenceUrl = {
  geral: [
    'https://beira.pt/portal/noticias/sociedade/associacao-juvenil-falcoes-da-europa-eurohawk-comemora-1o-aniversario/',
    'https://www.youtube.com/watch?v=9SLMInSmf2E&t=1s',
    'https://beira.pt/portal/noticias/sociedade/prevencao-legalizacao-das-drogas-debatidas-pinhel/',
    'https://beira.pt/portal/noticias/tertulia-poetica-do-falcao-em-pinhel/',
  ],
  festival: [
    'https://www.facebook.com/hawkfestival',
    'https://www.facebook.com/hawkfestival/videos/897959260357537/',
    'https://www.facebook.com/hawkfestival/videos/894168217403308/',
    'https://www.facebook.com/hawkfestival/videos/892990927521037/',
    'https://www.facebook.com/hawkfestival/videos/892260564260740',
    'https://beira.pt/portal/noticias/sociedade/hawk-festival-17-arranca-hoje-pinhel/',
    'https://ointerior.pt/arquivo/hawk-festival-comeca-hoje-em-pinhel/',
  ],
  erasmus: [
    'https://videos.sapo.pt/F3zgtySEfj0pGb0oAiu5',
    'https://www.youtube.com/watch?v=9SLMInSmf2E&t=1s',
    'https://ointerior.pt/arquivo/35-jovens-europeus-debatem-mobilidade-juvenil/',
    'https://beira.pt/portal/noticias/sociedade/formacao-sobre-procedimentos-e-candidaturas-aos-programas-erasmus-juventude-em-acao-decorre-em-pinhel/',
  ],
  report: ['https://m.facebook.com/story.php?story_fbid=1456432061527386&id=270919800078624'],
  news: [
    'https://www.altitude.fm/actualidade/fnee-promove-ajuda-humanitaria-juntos-pela-ucrania/',
    'https://www.altitude.fm/actualidade/camioes-com-ajuda-humanitaria-rumam-em-direcao-a-ucrania/',
    'https://ointerior.pt/sociedade/cerca-de-80-toneladas-de-bens-essenciais-partiram-da-guarda-para-a-ucrania/',
    'https://www.cm-celoricodabeira.pt/ajuda-humanitaria-juntos-pela-ucrania-vamos-encher-um-camiao-da-guarda-com-ajuda-humanitaria/',
    'https://politecnicoguarda.pt/politecnico-da-guarda-recolhe-bens-para-acao-humanitaria-na-ucrania/',
  ],
};

const humanitarianSlideShowImages = [
  { url: humanitarian_1 },
  { url: humanitarian_2 },
  { url: humanitarian_3 },
  { url: humanitarian_4 },
  { url: humanitarian_5 },
] as SlideshowImage[];

export { historyReferenceUrl, humanitarianSlideShowImages };
